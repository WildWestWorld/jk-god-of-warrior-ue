...
struct FWarriorInputActionConfig
{
	...
	// 检查输入配置是否有效
	// @return bool - 当InputTag有效且InputAction不为空时返回true，否则返回false
	bool IsValid() const 
	{
...
/**
 * 这个类是用于配置输入操作的DataAsset类，可以在编辑器中创建配置文件，用于保存输入配置数据。
 * 这样我们就能在 编辑器Data_Asset 中拿到这个我们自定义的类了
 */
UCLASS()
